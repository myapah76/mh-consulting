...
import type { AdminServiceSummary } from '../types/service.types';
                ...
                <tr key={service.id} className="hover:bg-gray-50/70">
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <LucideIcon name={service.icon ?? 'Briefcase'} size={18} />
                      </span>
                      <span className="max-w-xs font-bold text-gray-900">{service.title}</span>
                    </div>
                  </td>
                  <td className="px-5 py-4 text-gray-600">{service.categoryName}</td>
                  <td className="px-5 py-4">
                    <span className={`rounded-full px-2.5 py-1 text-xs font-bold ${service.active ? 'bg-green-100 text-green-700' : 'bg-gray-200 text-gray-600'}`}>
                      {service.active ? 'Đang hoạt động' : 'Không hoạt động'}
                    </span>
                  </td>
                  <td className="px-5 py-4">
                        <div className="flex justify-end gap-2">
                          {service.active && <Link to={`/dich-vu/${service.slug}`} target="_blank" className="rounded-md border border-gray-200 px-2.5 py-1.5 text-xs font-bold text-gray-600 hover:text-[#d40000]">Xem</Link>}
                          <Link to={`/admin/services/${service.id}/edit${searchParams.toString() ? `?return=${encodeURIComponent(searchParams.toString())}` : ''}`} className="rounded-md border border-gray-200 px-2.5 py-1.5 text-xs font-bold text-gray-600 hover:text-[#d40000]">Sửa</Link><button type="button" disabled={mutationPending} onClick={() => setPendingAction({ type: 'active', service })} className="rounded-md border border-gray-200 px-2.5 py-1.5 text-xs font-bold text-gray-600 hover:text-[#d40000] disabled:opacity-50">{service.active ? 'Ẩn' : 'Kích hoạt'}</button><button type="button" disabled={mutationPending} onClick={() => setPendingAction({ type: 'delete', service })} className="rounded-md border border-red-200 px-2.5 py-1.5 text-xs font-bold text-red-600 hover:bg-red-50 disabled:opacity-50">Xóa</button></div></td></tr>
              ))}</tbody>
            </table>
          </div>
        )}
